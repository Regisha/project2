<?php
// SMSC.RU API (www.smsc.ru) версия 2.2 (10.08.2011)

define("SMSC_LOGIN", $smsc['login']);		// логин клиента
define("SMSC_PASSWORD", $smsc['pass']);	// пароль или MD5-хеш пароля в нижнем регистре
...
{
	return mail("send@send.smsc.ru", "", SMSC_LOGIN.":".SMSC_PASSWORD.":$id:$time:$translit,$format,$sender:$phones:$message", "Content-Type: text/plain; charset=".SMSC_CHARSET."\n");
}
...
function get_sms_cost($phones, $message, $translit = 0, $sender = false, $query = "")
{
	$m = _smsc_send_cmd("send", "cost=1&phones=".urlencode($phones)."&mes=".urlencode($message).($sender === false ? "" : "&sender=".urlencode($sender)).
					"&charset=".SMSC_CHARSET."&translit=$translit".($query ? "&$query" : ""));
...
//https://smsc.ru/sys/info.php?get_operator=1&login=demo&psw=demo&phone=79284312242&fmt=1
function get_operator($tel)
{
	$m = _smsc_send_cmd("info",'get_operator=1&phone=7'.$tel);
	$array = isset($m[0]) ? array ('operator' => $m[0],'kray' => $m[1]) : false;
	return $array;
}
